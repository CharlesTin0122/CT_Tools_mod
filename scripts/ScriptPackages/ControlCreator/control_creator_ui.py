...
class ControlCreatorUI:
    def __init__(self):
        ...
        self.current_rgb_color = pm.colorIndex(self.default_color_index, q=True)
        ...
        # --- 菜单 ---
        pm.menu(label="文件", tearOff=False)
        pm.menuItem(label="刷新控制器列表", command=self.populate_shapes_grid)
        pm.menuItem(label="设置控制器形状文件夹...", command=self.set_shapes_folder_cmd)
        pm.menuItem(divider=True)
        pm.menuItem(label="关闭", command=lambda *args: pm.deleteUI(self.window_name))

        # --- 主布局 ---
        self.main_layout = pm.columnLayout(adj=True, rs=5, cal="center")

        # --- 搜索栏 (简化) ---
        pm.text(label="搜索（暂未实现动态过滤）：")
        self.search_field = pm.textField(placeholderText="搜索控制器形状...")

        # --- 控制器形状网格 ---
        pm.separator(h=10, style="in")
        pm.text(label="选择控制器形状:")
        self.shapes_scroll_layout = pm.scrollLayout(
            h=200, childResizable=True, borderVisible=True
        )  # 固定高度，可滚动
        self.shapes_grid_layout = pm.gridLayout(
            numberOfColumns=3, cellWidthHeight=(100, 100), ag=True
        )  # 3列，单元格大小
        self.populate_shapes_grid()
        pm.setParent("..")  # 返回到 shapes_scroll_layout 的父级 (main_layout)
        pm.setParent("..")  # 返回到 main_layout 的父级
        ...
        with pm.frameLayout(
            ...
            with pm.columnLayout(adj=True, rs=3):
                self.ctrl_name_field = pm.textFieldGrp(
                ...
                with pm.rowLayout(
                    ...
                    pm.text(label="颜色:")
                    self.color_swatch = pm.canvas(
                        width=50,
                        height=20,
                        rgbValue=self.current_rgb_color,
                        pressCommand=self.pick_color_cmd,
                    )
                    self.color_mode_radio = pm.radioButtonGrp(
                        label="",
                        labelArray2=["索引", "RGB"],
                ...
                self.scale_field = pm.floatFieldGrp(
                    label="大小 (缩放): ",
                    value1=1.0,
                    numberOfFields=1,
                    cw=[(1, 80), (2, 50)],
                ...
                # 旋转 (简化为三个轴向的输入)
                self.rotate_x_field = pm.floatFieldGrp(
                    label="旋转 X: ",
                    value1=0.0,
                    numberOfFields=1,
                    cw=[(1, 80), (2, 50)],
                )
                self.rotate_y_field = pm.floatFieldGrp(
                    label="旋转 Y: ",
                    value1=0.0,
                    numberOfFields=1,
                    cw=[(1, 80), (2, 50)],
                )
                self.rotate_z_field = pm.floatFieldGrp(
                    label="旋转 Z: ",
                    value1=0.0,
                    numberOfFields=1,
                    cw=[(1, 80), (2, 50)],
        ...
        # --- 操作按钮 ---
        self.match_to_selection_cb = pm.checkBox(
    ...
    def set_shapes_folder_cmd(self, *args):
        ...
        if result and len(result) > 0:
            ...
            if new_path.is_dir():
                ...
                self.populate_shapes_grid()
                pm.displayInfo(f"控制器形状文件夹已更新为: {new_path}")
            else:
                pm.warning("选择的路径不是一个有效的文件夹。")
    ...
    def update_color_mode(self, *args):
        ...
        if mode == 1:  # 索引
            pm.displayInfo("颜色模式: 索引 (下次拾取颜色将设置索引)")
        else:  # RGB
            pm.displayInfo("颜色模式: RGB (下次拾取颜色将直接使用RGB)")
    ...
    def pick_color_cmd(self, *args):
        ...
        # 打开 Maya 颜色编辑器
        pm.colorEditor(rgb=initial_color)  # 用之前swatch的颜色初始化

        if pm.colorEditor(query=True, result=True):  # 如果用户点击了 "OK"
            rgb_color = pm.colorEditor(query=True, rgb=True)  # 获取 RGB 值 (0-1范围)
            ...
            if current_mode_is_index:
                ...
                for i in range(32):  # Maya有32个标准索引颜色
                    index_rgb = pm.colorIndex(i, q=True)
                ...
                self.color_swatch.setBackgroundColor(
                    pm.colorIndex(self.default_color_index, q=True)
                )
                pm.displayInfo(f"选择了索引颜色: {self.default_color_index}")
            else:  # RGB 模式
                self.color_swatch.setBackgroundColor(rgb_color)
                pm.displayInfo(f"选择了RGB颜色: {rgb_color}")
        else:  # 用户取消
            pm.displayInfo("颜色选择已取消。")
    ...
    def populate_shapes_grid(self, *args):
        ...
        if (
            ...
            pm.warning(f"控制器形状文件夹未找到或不是目录: {self.control_shapes_path}")
            pm.text(
    ...
    def select_shape_cmd(self, shape_file_path, *args):
        ...
        pm.displayInfo(f"已选择形状: {shape_file_path.name}")
    ...
    def create_controls_cmd(self, *args):
        ...
        if not loaded_data:
            pm.error(f"无法加载形状数据: {self.selected_shape_file.name}")
            return
        ...
        pm.displayInfo(f"控制器 '{created_info['offset_group'].name()}' 已创建。")

        # 应用后处理 (大小、旋转、匹配)
        self.apply_post_process(created_info)

        pm.select(created_info["offset_group"])
    ...
    def apply_post_process(self, controller_info):
        ...
        if self.match_to_selection_cb.getValue():
            ...
            if selection and isinstance(
                selection[0], (pm.nodetypes.Joint, pm.nodetypes.Transform)
            ):
                target_joint_or_obj = selection[0]
                match_to_joint(controller_info, target_joint_or_obj)
                pm.displayInfo(f"控制器已匹配到: {target_joint_or_obj.name()}")
            else:
                pm.warning("请求匹配，但未选择有效的目标骨骼或对象。")

        pm.select(offset_grp)

    def apply_post_process_to_selected_cmd(self, *args):
        """对场景中选中的控制器（或其offset group）应用UI中的大小和旋转设置"""
        selected_nodes = pm.selected(type="transform")
        ...
        for node in selected_nodes:
            ...
            if node.name().endswith("_offset"):  # 假设选中了offset group
                ...
                pm.warning(f"跳过节点 {node.name()}：无法确定其为控制器结构。")
                continue

            if (
                "curve_transform" not in temp_info and "offset_group" not in temp_info
            ):  # 确保至少有一个有效
                pm.warning(f"跳过节点 {node.name()}：无法识别为有效控制器组件。")
                continue

            pm.displayInfo(f"对选中的 {node.name()} 应用后处理...")
