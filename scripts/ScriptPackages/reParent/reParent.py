...
pm.menuItem(
    c=lambda *args: pm.mel.BakeAndDelete_reParent(),
...
pm.button(
    "reParentButton",
    h=40,
    c=lambda *args: pm.mel.reParentStarter(),
    bgc=(0.8, 0.8, 0.8),
    l="reParent",
    w=120,
)
...
pm.button(
    h=40, c=lambda *args: pm.mel.manualModeGo(), bgc=(0.8, 0.8, 0.8), l="Go", w=40
)
pm.button(
    h=40,
    c=lambda *args: pm.mel.manualModeCancel(),
    bgc=(0.22, 0.22, 0.22),
    l="Cancel",
    w=60,
)
...
def reParent():
    ...
    if not pm.objExists("reParent_sets"):
        pm.sets(em=1, name="reParent_sets")
        # Create Sets
    ...
    if pm.objExists("All_Sessions_reParentControls_set"):
        ...
        pm.sets(name="All_Sessions_reParentControls_set")
        pm.sets("All_Sessions_reParentControls_set", edit=1, fe="reParent_sets")

    if pm.objExists("Last_Session_reParentControls_set"):
        pm.delete("Last_Session_reParentControls_set")
        pm.sets(name="Last_Session_reParentControls_set")
        pm.sets(
    ...
    if pm.objExists("Last_Session_reParentLocator_set"):
        ...
        pm.sets(name="Last_Session_reParentLocator_set")
    ...
    if not pm.objExists("All_Session_reParentLocator_set"):
        pm.sets(em=1, name="All_Session_reParentLocator_set")
    ...
    for SelCtrl in SelectedControls:
        if (
            pm.getAttr((str(SelCtrl) + ".tx"), keyable=1) == 1
            and pm.getAttr((str(SelCtrl) + ".tx"), lock=1) == 0
        ):
            pm.select((str(SelCtrl) + "_ReParent_Locator"), SelCtrl)
            pm.pointConstraint(weight=1, n=(str(SelCtrl) + "ReParent"))

        if (
            pm.getAttr((str(SelCtrl) + ".rx"), keyable=1) == 1
            and pm.getAttr((str(SelCtrl) + ".rx"), lock=1) == 0
        ):
            pm.select((str(SelCtrl) + "_ReParent_Locator"), SelCtrl)
...
def reParentManualStarter():
    ...
    int(pm.checkBox("PinCheckBox", q=1, v=1))
    SelCtrl = ""
    SelectedControls = pm.ls(sl=1)
    if pm.objExists("TempLocator"):
        pm.delete("TempLocator")
...
def manualModeCancel():
    if (pm.window("ManualWindow", ex=1)):
...
def manualModeGo():
    ...
    for SelCtrl in SelectedControls:
        if PinButton == 0:
            pm.select(SelCtrl, (str(SelCtrl) + "_ReParent_Locator"), r=1)
    ...
    for SelCtrl in SelectedControls:
        if (
            pm.getAttr((str(SelCtrl) + ".tx"), keyable=1) == 1
            and pm.getAttr((str(SelCtrl) + ".tx"), lock=1) == 0
            and pm.getAttr((str(SelCtrl) + ".rx"), keyable=1) == 1
            and pm.getAttr((str(SelCtrl) + ".rx"), lock=1) == 0
        ):
            pm.select((str(SelCtrl) + "_ReParent_Locator"), SelCtrl)
...
def reParentRelative():
    ...
    for r in range(0, amountOfAllCtrls - 1):
        if (
            pm.getAttr((SelectedControls[r] + ".tx"), keyable=1) == 1
            and pm.getAttr((SelectedControls[r] + ".tx"), lock=1) == 0
        ):
            pm.select((SelectedControls[r] + "_ReParent_Locator"), SelectedControls[r])
            pm.pointConstraint(weight=1, n=(SelectedControls[r] + "ReParent"))

        if (
            pm.getAttr((SelectedControls[r] + ".rx"), keyable=1) == 1
            and pm.getAttr((SelectedControls[r] + ".rx"), lock=1) == 0
        ):
            pm.select((SelectedControls[r] + "_ReParent_Locator"), SelectedControls[r])
...
def reParentStayHere():
    ...
    pm.delete((SelectedControls[0]), constraints=1)
    for SelCtrl in SelectedControls:
        if (
            pm.getAttr((str(SelCtrl) + ".tx"), keyable=1) == 1
            and pm.getAttr((str(SelCtrl) + ".tx"), lock=1) == 0
...
def IKmode():
    ...
    # create Joints
    for i in range(0, 3):
        WorldTr = pm.xform((SelectedControls[i] + "_reParentIKlocator"), q=1, ws=1, t=1)
        pm.joint(
    ...
    PoleVectorLengths = float(
        (pm.getAttr(SelectedControls[1] + "_reParentIKJoint.translateY"))
        + (pm.getAttr(SelectedControls[2] + "_reParentIKJoint.translateY"))
    )
    PoleVectorMult = float(
        (pm.getAttr(SelectedControls[2] + "_reParentIKJoint.translateY"))
        / (pm.getAttr(SelectedControls[1] + "_reParentIKJoint.translateY"))
    )
    firstRePArentIK_locator_vector = pm.xform(
        (SelectedControls[0] + "_reParentIKlocator"), q=1, ws=1, t=1
    )
    secondRePArentIK_locator_vector = pm.xform(
        (SelectedControls[1] + "_reParentIKlocator"), q=1, ws=1, t=1
    )
    thindRePArentIK_locator_vector = pm.xform(
    ...
    pm.setAttr((SelectedControls[1] + "_reParentIKPole.translateX"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKPole.translateY"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKPole.translateZ"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKPole.rotateX"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKPole.rotateY"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKPole.rotateZ"), 0)
    pm.duplicate(
        (SelectedControls[1] + "_reParentIKPole"),
    ...
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.translateX"), 0)
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.translateY"), 0)
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.translateZ"), 0)
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.rotateX"), 0)
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.rotateY"), 0)
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.rotateZ"), 0)
    pm.parent(
        (SelectedControls[0] + "_reParentIKoffset"),
    ...
    pm.parent((SelectedControls[1] + "_reParentIKPole"), w=1)
    pm.parentConstraint(
        (SelectedControls[1] + "_reParentIKlocator"),
        (SelectedControls[1] + "_reParentIKPole"),
        mo=1,
    ...
    # Locked attrs
    if (
        pm.getAttr((SelectedControls[1] + ".rotateX"), l=1) == 1
        or pm.getAttr((SelectedControls[1] + ".rotateY"), l=1) == 1
        or pm.getAttr((SelectedControls[1] + ".rotateZ"), l=1) == 1
    ):
        ...
        if pm.getAttr((SelectedControls[1] + ".rotateX"), l=1) == 0:
            pm.melGlobals["LockedAttr1"] = "y"
            pm.melGlobals["LockedAttr2"] = "z"

        if pm.getAttr((SelectedControls[1] + ".rotateY"), l=1) == 0:
            pm.melGlobals["LockedAttr1"] = "x"
            pm.melGlobals["LockedAttr2"] = "z"

        if pm.getAttr((SelectedControls[1] + ".rotateZ"), l=1) == 0:
            pm.melGlobals["LockedAttr1"] = "y"
            pm.melGlobals["LockedAttr2"] = "x"

        pm.setAttr((SelectedControls[1] + "tempLockedCtrl.rotateX"), k=1)
        pm.setAttr((SelectedControls[1] + "tempLockedCtrl.rotateY"), k=1)
        pm.setAttr((SelectedControls[1] + "tempLockedCtrl.rotateZ"), k=1)
        pm.setAttr((SelectedControls[1] + "tempLockedCtrl.rotateX"), lock=0)
        pm.setAttr((SelectedControls[1] + "tempLockedCtrl.rotateY"), lock=0)
        pm.setAttr((SelectedControls[1] + "tempLockedCtrl.rotateZ"), lock=0)
        ...
        if SelectedControls[0] != pm.melGlobals["UpHierarchyObject"][0]:
            ...
            pm.parentConstraint(
                pm.melGlobals["UpHierarchyObject"][0],
        ...
        if (
            LocalPinButton == 1
            and SelectedControls[0] != pm.melGlobals["UpHierarchyObject"][0]
        ):
            pm.delete((SelectedControls[0] + "_reParentIK_offset_grp"), constraints=1)
            pm.parentConstraint(
                pm.melGlobals["UpHierarchyObject"][0],
        ...
        pm.transformLimits((SelectedControls[1] + "tempLockedCtrl"), erx=(0, 0))
        pm.transformLimits((SelectedControls[1] + "tempLockedCtrl"), ery=(0, 0))
        pm.transformLimits((SelectedControls[1] + "tempLockedCtrl"), erz=(0, 0))
        ...
        pm.orientConstraint(
            (SelectedControls[1] + "tempLockedCtrl"),
            SelectedControls[1],
            skip=[pm.melGlobals["LockedAttr1"], pm.melGlobals["LockedAttr2"]],
            mo=1,
            weight=1,
        )
        pm.orientConstraint(
    ...
    pm.setAttr((SelectedControls[0] + "_reParentIKJoint.drawStyle"), 2)
    # visibility
    pm.setAttr((SelectedControls[1] + "_reParentIKJoint.drawStyle"), 2)
    pm.setAttr((SelectedControls[2] + "_reParentIKJoint.drawStyle"), 2)
    pm.setAttr((SelectedControls[0] + "_reParentIKlocator.visibility"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKlocator.visibility"), 0)
    pm.setAttr((SelectedControls[0] + "_reParentIKoffset.visibility"), 0)
    pm.setAttr((SelectedControls[1] + "_reParentIKoffset.visibility"), 0)
    pm.setAttr((SelectedControls[1] + "_ikHandle.visibility"), 0)
    # size
    JointLentgts = float(
        pm.getAttr(SelectedControls[1] + "_reParentIKJoint.translateY")
    )
    ...
    # color
    pm.setAttr((SelectedControls[2] + "_reParentIKlocatorShape.overrideEnabled"), 1)
    pm.setAttr((SelectedControls[2] + "_reParentIKlocatorShape.overrideColor"), 17)
    pm.setAttr((SelectedControls[1] + "_reParentIKPoleShape.overrideEnabled"), 1)
    pm.setAttr((SelectedControls[1] + "_reParentIKPoleShape.overrideColor"), 13)
    pm.sets(
        (SelectedControls[2] + "_reParentIKlocator"),
        edit=1,
    ...
    pm.select((SelectedControls[2] + "_reParentIKlocator"), r=1)
...
def reParentLocatorSize():
    ...
    # Clean Joints
    if (
        pm.objectType(SelectedControls[0]) == "joint"
        and not pm.objExists(SelectedControls[0] + "Shape")
        and not pm.objExists(SelectedControls[0] + "Shape1")
    ):
        firstPos = []
        secondPos = []
        firstVect = []
        ...
        if SelectedControls[0] != pm.melGlobals["UpHierarchyObject"][0]:
            ...
            pm.setAttr("TempLocatorShape.localScaleX", (firstLen * 2))
            pm.setAttr("TempLocatorShape.localScaleY", (firstLen * 2))
            pm.setAttr("TempLocatorShape.localScaleZ", (firstLen * 2))
            ...
            if SelectedControls[0] != pm.melGlobals["UpHierarchyObject"][0]:
                firstPos = pm.xform(SelectedControls[0], q=1, ws=1, t=1)
                secondPos = pm.xform(
                    pm.melGlobals["UpHierarchyObject"][0], q=1, ws=1, t=1
                )
                firstVect = secondPos - firstPos
                firstLen = float(
                    pm.mel.sqrt(
    ...
    if pm.objectType(SelectedControls[0]) == "joint" and pm.objExists(
        ...
        pm.setAttr("TempLocatorShape.localScaleX", (locatorSize / 1))
        pm.setAttr("TempLocatorShape.localScaleY", (locatorSize / 1))
        pm.setAttr("TempLocatorShape.localScaleZ", (locatorSize / 1))
