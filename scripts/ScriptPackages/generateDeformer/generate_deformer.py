...
def ShowUI():
    ...
    objList = cmd.textScrollList(
        "objList", allowMultiSelection=True, height=215, width=190
    )
    ...
    objListOther = cmd.textScrollList(
        "objListOther", allowMultiSelection=True, height=215, width=190
    )
    ...
    BaseObject = cmd.button(
        label="对象",
        c="sel=cmd.ls(sl=1);cmd.textField('BaseObj_field', e=1 ,fi = sel[0])",
    )
